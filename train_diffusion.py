...
def train_diffusion(args):
    ...
    unet_model = UNet(
        ...
        num_resnet_blocks=1,
        context_dim=args.unet_dim_mults[-1] * args.unet_base_dim,
        attn_heads=8
    ).to(device)
    ...
    if args.weights_path is not None and os.path.exists(args.weights_path):
        start_epoch, best_loss, _, _ = diffusion_helper.load_checkpoint_for_resume(device='cuda', model=unet_model, optimizer=optimizer, checkpoint_path=args.weights_path)
...
if __name__ == "__main__": 
    ...
    # UNet args
    parser.add_argument('--num_resnet_block', type=int, default=1, help="Number of resnet block in each U-net layer")
    ...
    parser.add_argument('--continue_log_dir', type=str, default=None, help='Directory for continue logging on old TensorBoard logs')
    parser.add_argument('--continue_checkpoint_dir', type=str, default=None, help='Directory for continue training on old checkpoints')
    # Loading model args
